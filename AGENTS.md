...
from gjs.guide, vendored verbatim so it applies to anything editing this repo.
Refresh it from [the upstream Markdown][best-practices-markdown] when upstream
changes.

## This project

Read the rules below before changing anything under `src/tailscale@hagreli.us/`,
with two standing exceptions:

**`daemon/cli.js` and `ui/launcher.js` spawn subprocesses on purpose.** Tailscale
exposes no D-Bus interface. Prefs, status and the IPN event bus already go
through the LocalAPI unix socket (`daemon/localApi.js`), which is the closest
thing available. `serve` and `funnel` stay on the CLI because it validates ports,
provisions certificates and enforces tailnet funnel policy; the terminal spawn
exists because `tailscale up` prints an auth URL and blocks. Both modules say so
at the top — keep those comments, they are what a reviewer looks for.

**Optional chaining on nullable data is not the "unnecessary check" the rules
below forbid.** `this._prefs?.ExitNodeID ?? ''` guards a field that is genuinely
null until the first daemon response. The rule targets `?.()` and
`typeof x === 'function'` on APIs that are guaranteed to exist. Likewise, every
`try`/`catch` here wraps a call that throws `GError` — `Gio.Subprocess.new`,
`GLib.shell_parse_argv`, `JSON.parse`, promisified async GIO. Do not strip them.
